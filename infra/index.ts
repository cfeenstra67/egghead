...
async function staticWebsite({
  ...
  const certiciateValidation = new aws.acm.CertificateValidation(
    `${idPrefix}certificate-validation`,
    {
...
function githubRolePolicy(bucketArns: string[]): string {
  const readActions = ["iam", "s3", "acm", "route53", "cloudfront"].flatMap(
    (service) => [
      `${service}:Get*`,
      `${service}:List*`,
      `${service}:Describe*`,
