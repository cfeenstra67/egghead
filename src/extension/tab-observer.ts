...
export class TabObserver {
  ...
  handleNavigationComplete(event: NavigationObserver.NavigationEvent): void {
    ...
    getTabById(event.detail.tabId as number, async (tab) => {
      const response = await this.server.tabChanged(tabChangedRequestFromTab({
        tab,
  ...
  async handleTabRemoved(tabId: number, removeInfo: chrome.tabs.TabRemoveInfo): Promise<void> {
  ...
  async cleanupSessions(): Promise<void> {
    ...
    await Promise.all(diff.updates.map(async (update) => {
      await this.server.tabChanged(update);
    }));
    await Promise.all(diff.deletes.map(async (tabId) => {
  ...
  diffCurrentState(): Promise<TabStateDiff> {
    return new Promise((resolve, reject) => {
      chrome.tabs.query({ status: 'complete' }, async (tabs) => {
        ...
        const currentActiveSessionTabIds = new Set(results.map((session) => session.tabId));
